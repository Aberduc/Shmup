...
public class Enemy extends MovingGoal {
    ...
    public void shot() {
        ...
        if (lives <= 0) {
            ...
            this.setSpeed(screenY / 6 * ((float) Math.pow(0.95, lives - 1)));
        }
    }

    public static void mergeEnemies(Enemy e1, Enemy e2) {
        if (e1.isActive && e2.isActive) {
            if(e1.lives >= e2.lives){
                e1.lives = e1.lives + e2.lives;
                e2.isActive = false;
                e1.setRadius(e1.screenX / 30 * ((float) Math.sqrt(e1.lives)));
                e1.setSpeed(e1.screenY / 6 * ((float) Math.pow(0.95, e1.lives - 1)));
            } else {
                e2.lives = e1.lives + e2.lives;
                e1.isActive = false;
                e2.setRadius(e2.screenX / 30 * ((float) Math.sqrt(e2.lives)));
                e2.setSpeed(e2.screenY / 6 * ((float) Math.pow(0.95, e2.lives - 1)));
            }
        }
    }
